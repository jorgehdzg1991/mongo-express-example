...
const { OK, INTERNAL_SERVER_ERROR, NOT_FOUND, UNAUTHORIZED, BAD_REQUEST } = require('http-status-codes');
...
router.post('/refreshAuthToken', async (req, res) => {
    try {
        ...
        if (!authToken) {
            return respond(res, BAD_REQUEST, 'Missing auth jwt token header.');
        }
